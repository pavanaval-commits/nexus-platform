  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Welcome back to Nexus</h1>
          <p className="text-muted-foreground mt-1">
            Your regulatory intelligence hub - Track, analyze, and connect across the regulatory landscape
          </p>
        </div>
            ...
            {upcomingDeadlines.map((deadline, index) => (
              <div key={index} className="p-3 border border-border rounded-lg">
                ...
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  ...
                  <span className="font-medium">{deadline.daysLeft} days left</span>
                </div>
                <Progress value={(30 - deadline.daysLeft) / 30 * 100} className="mt-2" />
              </div>
            ))}
            <Button variant="ghost" className="w-full text-sm">
              <Calendar className="h-4 w-4 mr-2" />
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {topConsultants.map((consultant, index) => (
              <div key={index} className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent/50 transition-colors cursor-pointer">
                <Avatar>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{consultant.firm}</p>
                  <p className="text-xs text-blue-600 mt-1">{consultant.specialization}</p>
                </div>
                <Button variant="outline" size="sm">
                  Contact
                </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="pt-4 border-t border-border">
              <h4 className="font-medium text-sm mb-3">Regulatory Insights</h4>
              <div className="space-y-2">
                <div className="p-2 bg-blue-50 rounded-lg border-l-2 border-blue-200">
                  <p className="text-xs font-medium text-blue-800">📊 147 new regulatory updates this week</p>
                </div>
                <div className="p-2 bg-green-50 rounded-lg border-l-2 border-green-200">
                  <p className="text-xs font-medium text-green-800">✅ 94% compliance score - Above industry average</p>
                </div>
                <div className="p-2 bg-orange-50 rounded-lg border-l-2 border-orange-200">
                  <p className="text-xs font-medium text-orange-800">⏰ 3 critical deadlines approaching</p>
                </div>
              </div>
            </div>
          </CardContent>
